...
import type {
  ...
  SiteModule, SiteModuleOptions, SiteMutationResult, SiteView, SiteOverview,
} from '.'
...
function canonicalPath(path: string) {
  ...
  if (problem) throw new DomainError(problem.code, problem.message)
  return path
}
...
export async function createSiteModule(config: AppConfig, database: AppDatabase, options: SiteModuleOptions = {}): Promise<SiteModule> {
  ...
  const createSite: SiteModule['createSite'] = (principal, command) => mutation(principal, command.operationId, undefined, async () => {
    ...
    // The fingerprint records what the caller asked for: an omitted expiration
    // replays as the same request even after the owner changes the default.
    const expires = requestedExpires === undefined ? 'default' : requestedExpires
    ...
    try {
      ...
      try {
        await revisionGate.run(async () => sql.transaction(() => {
          const timestamp = now()
          // The owner default is resolved in the transaction that commits the site.
          const lifetime = requestedExpires === undefined ? ownerSettings(principal.ownerId).defaultExpiresInSeconds : requestedExpires
          const row: SiteRow = { id: siteId, owner_id: principal.ownerId, name, visibility: 'private', visibility_generation: 1, version: 1,
            ...
            expires_at_ms: lifetime === null ? null : timestamp + lifetime * 1000, deletion_reason: null, size_bytes: total, file_count: sorted.length }
  ...
  const changeFiles = (kind: 'write' | 'delete-files', principal: Principal, command: { operationId: string; siteId: string; expectedVersion: number; files?: readonly FileInput[]; paths?: readonly string[]; maximumUploadBytes?: number; requireExisting?: boolean }): Promise<FileMutationResult> =>
    mutation(principal, command.operationId, command.siteId, async () => {
      ...
      if (immediateEntries && authorizedRow) {
        ...
        if (command.requireExisting && immediateEntries.some((entry) => !currentFiles.has(entry.path))) throw new DomainError('NOT_FOUND', 'File not found')
  ...
  return {
    editFile: (principal, command) => changeFiles('write', principal, { ...command,
      files: [{ path: command.path, content: command.content }], requireExisting: true }),
    async getOverview(principal) {
      const timestamp = now()
      const soon = timestamp + 72 * 60 * 60 * 1000
      const live = "owner_id=? AND lifecycle='active' AND (expires_at_ms IS NULL OR expires_at_ms>?)"
      // Synchronous queries use one timestamp and cannot interleave with mutations.
      const totals = sql.prepare(`SELECT count(*) activeSites,
    ...
    async openCurrentFile(principal, query) {
      let site!: SiteView
      const lease = await acquireRevision(() => {
        const row = siteRow(principal.ownerId, query.siteId)
        site = view(row)
      ...
      return { site, file: await lease.open(query.path) }
    },
