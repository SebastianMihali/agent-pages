    ...
    try {
      ...
      if (siteId && resource === 'files' && request.method === 'PUT') {
        if (request.headers.get('content-type')?.split(';')[0].trim().toLowerCase() === 'multipart/form-data') {
          const upload = await readMultipart(request, config, { fields: schemas.delete })
          try {
            return json(await sites.writeFiles(principal, { siteId, operationId: upload.manifest.operationId,
              expectedVersion: upload.manifest.expectedVersion, files: upload.files, maximumUploadBytes: upload.maximumUploadBytes }))
