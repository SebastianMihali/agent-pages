...
export async function readMultipart(request: Request, config: AppConfig) {
  ...
  const manifestSchema = schemas.delete.extend({ files: z.array(z.strictObject({
    path: z.string().min(1).max(512), partName: z.string().regex(/^[a-zA-Z0-9_-]{1,64}$/),
  })).min(1).max(config.limits.maxBatchFiles) })
  ...
  const deadline = setTimeout(() => fail(new DomainError('BUSY', 'Multipart upload exceeded its 30-second deadline')), 30_000)
  deadline.unref()
  ...
  try {
    ...
    return {
      operationId: value.operationId, expectedVersion: value.expectedVersion, files, maximumUploadBytes: maximumBytes,
      async close() {
