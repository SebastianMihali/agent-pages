...
To return a site to earlier content, call `list_site_revisions` and select a retained `revisionId`. Call `restore_site_revision` with that ID, the current `expectedVersion` and a new `operationId`. It reactivates the immutable revision at the same URL and preserves visibility and expiration. A different active revision increments the site version; restoring the already active revision checks the version and returns a no-op receipt. A retired revision is unavailable. History is owner-only, even for public sites. The configured `historyLimit` counts previous revisions beyond the active one; 0 means no retained history. A pinned file read works only while the revision and its files remain available, so treat cursors as short-lived.
