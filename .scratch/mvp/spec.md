...
- Site responses use `X-Content-Type-Options: nosniff`, `Referrer-Policy: no-referrer`, `Cross-Origin-Resource-Policy: same-origin`, `X-Robots-Tag: noindex, nofollow` and document `frame-ancestors 'none'`. Every response on a content host, including denials, redirects, HEAD and 404, is excluded from search indexing; there is no per-site option. Disable camera, microphone, geolocation and legacy `document.domain` relaxation via the appropriate browser policies; send `Origin-Agent-Cluster: ?1` on documents. Verify cross-site behavior in the supported browser tests.
