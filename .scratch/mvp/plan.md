...
Updated: 2026-09-05. Local implementation and verification complete; remote deployment acceptance is deferred by the owner.
...
Tickets 01–06 contain the implemented behavior and its verification. Ticket 07 separates the completed local checks from deployment through the future Coolify target. Follow these ownership boundaries when extending the implementation.
...
| --- | --- | --- | --- |
| [01](issues/01-runtime-and-http-integration.md) | Small bootable TanStack/Node application, host dispatch and production MCP transport probe | — | done |
| [02](issues/02-owner-identity.md) | One owner, sessions, personal keys and shared authorization | 01 | done |
| [03](issues/03-site-publication.md) | Revision storage, site lifecycle, retry/concurrency and quotas | 02 | done |
| [04](issues/04-content-access.md) | Private-site browser handoff, content serving and explicit public visibility | 03 | done |
| [05](issues/05-rest-and-mcp.md) | Complete shared REST/MCP operations and binary upload | 04 | done |
| [06](issues/06-owner-interface.md) | Minimal owner UI and visibility/key controls | 05 | done |
...
The owner explicitly requested completion of local verification before choosing deployment references. Deployment verification needs the actual application hostname, content base domain, control over wildcard DNS/TLS, a reachable Coolify target and a running Docker engine. Collect these when the owner resumes deployment; do not request or store credentials in this plan.
...
Local implementation and deployment acceptance are distinct milestones. The local milestone requires tickets 01–06 and all local checks in ticket 07. Full deployment acceptance requires the actual Coolify proxy, wildcard DNS and TLS checks as well. README describes the verified local implementation and explicitly retains this deployment limitation.
